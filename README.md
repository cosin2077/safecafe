...
If the current version is already published, the first run prepares the next patch version across the root package, safe-lite package, and frontend/CLI version constant, then stops for review and a manual commit. Run `pnpm release` again after committing to publish one build to IPFS and Cloudflare. Use `--bump=minor` or `--bump=major` when needed. When `.env` exists, the wizard uses it as the source of truth for Safecafe, Vite, Filebase, and the known Cloudflare Pages runtime secrets: non-empty values are uploaded and known secrets that are empty or missing are deleted. Without `.env`, it uses the current shell environment and never deletes unspecified Cloudflare secrets. `VITE_*` values are consumed during build. The release is a single linear process; if interrupted, run `pnpm release` again from a clean worktree. The wizard never updates ENS or commits release records automatically.
